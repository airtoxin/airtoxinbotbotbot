...
var Bot = (function() {
  return function() {
    ...
    self.watchEmergencyShutdown = function(dm) {
      ...
      if (dm.getText().indexOf(config.bot.emergency_shutdown_password) !== 0)
        return;
    ...
    self.watchFavorite = function(tweet) {
      if (tweet.getUserScreenName() === self.settings.screen_name) {
        return;
      }
      if (!brain.isFavoriteText(tweet.getFlatText())) {
        return;
      }
    ...
    brain.on("burst:favorited", function(userName, count) {
      if (count % config.favorite_event.burst_threshold !== 0) {
        return;
      }
      var tweetText =
        userName +
      ...
      setTimeout(function() {
        ...
        config.favorite_event.min_time,
        config.favorite_event.max_time
      ));
    });

    self.learn = function(tweet) {
      if (!_.includes(config.bot.teachers, tweet.getUserScreenName())) {
        return;
      }
      brain.learn(tweet);
    };
    ...
    self.initialize = function(callback) {
      async.each(
        config.bot.teachers,
        function(teacher, nextTeacher) {
          var maxID = null;
          async.eachSeries(
            _.range(1, 51),
            function(i, nextIndex) {
              var options = {
                screen_name: teacher,
                count: 200,
              ...
              client.get("statuses/user_timeline", options, function(
                error,
                tweets
              ) {
