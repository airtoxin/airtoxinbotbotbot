...
var Brain = (function() {
  return function() {
    ...
    self.keywords = [];
    self._errorHandle = function(error) {
      if (error) {
    ...
    self.isFavoriteText = function(text) {
      var keywords = _.union(
        config.bot.favorite.always_favorites,
        self.keywords
      );
      return _.some(
    ...
    self.memorizeTweet = function(tweet) {
      ...
      if (count >= config.tweet.burst_threshold) {
        self.emit("burst:tweet", key, count);
      }
    };
    ...
    self.memorizeFavorited = function(favoriteEvent) {
      ...
      if (val.count >= config.favorite_event.burst_threshold) {
        self.emit("burst:favorited", val.name, val.count);
      }
    };
    ...
    self.memorizeReplyed = function(tweet) {
      ...
      if (val.count >= config.reply.burst_threshold) {
        self.emit("burst:replyed", val.name, val.count);
      }
    };
    ...
    self._join = function(tokens) {
      return _.reduce(
        tokens,
        function(text, token) {
          if (text.match(/[a-zA-Z]$/) && token.match(/^[a-zA-Z]/)) {
            return text + " " + token;
          }
          return text + token;
        },
